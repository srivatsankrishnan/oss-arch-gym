...
flags.DEFINE_string('traject_dir', 
                    'randomsearch_FARSIsim_trajectories', 'Directory to save the dataset.')
...
flags.DEFINE_integer('seed', 110, 'hyperparameter1 for randomsearch')
...
def wrap_in_envlogger(env, envlogger_dir): 
    ...
    metadata = {
        'agent_type': 'RANDOMSEARCH_FARSIsim_EI',
...
def main(_):
    """We use random search algorithm on the action parameter space of FARSI simulator environment
    ...
    log_path = os.path.join(FLAGS.summary_dir, 'randomsearch__FARSI_sim_logs', FLAGS.reward_formulation, exp_name)
    ...
    mydesigner = random.RandomDesigner(problem.search_space, seed=FLAGS.seed)  #use this for RANDOM SEARCH
