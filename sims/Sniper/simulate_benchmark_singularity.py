...
class SniperLauncher:
    ...
    def prepare_cmd(self, pinpoint_path, output_path, config_path):
        ...
        #pinpoint_name = "perlbench.test_9806_t0r8_warmup1501_prolog0_region100000005_epilog0_008_0-09963.0.address"
        cmd = ['singularity', 'exec',
                'sniper_v3_v1.sif',
                '/home/sniper/run-sniper',
                '--power',
                '-d', str(complete_output),
                '-c', str(config_full_path),
                '--pinballs',pinpoint_path]
        print(cmd)
        return cmd
...
def combine_stats(output):
    try:
        ...
        for region in regions:
            ...
            try:  
                retry_count = 0
                # wait for sometime to see if power.py is generated
                while (retry_count < 5):
