...
flags.DEFINE_integer('num_steps_gs', 100, 'Number of training steps.')
flags.DEFINE_integer('num_episodes_gs', 2, 'Number of training episodes.')
flags.DEFINE_string('traject_dir_gs', 
                    'grid_search_trajectories', 
            'Directory to save the dataset.')
flags.DEFINE_bool('use_envlogger_gs', False, 'Use envlogger to log the data.')  
flags.DEFINE_string('summary_dir_gs', '.', 'Directory to save the summary.')
flags.DEFINE_string('reward_formulation_gs', 'power', 'Which reward formulation to use?')
FLAGS = flags.FLAGS
...
def wrap_in_envlogger(env, envlogger_dir):
    ...
    metadata = {
        ...
        'num_steps': FLAGS.num_steps_gs,
        'env_type': type(env).__name__,
    }
    if FLAGS.use_envlogger_gs:
...
def main(_):
    ...
    env = customenv_wrapper.make_custom_env(max_steps=FLAGS.num_steps_gs)
    fitness_hist = {}
    ...
    study = clients.Study.from_study_config(
     ...
     # experiment name 
    exp_name = "_num_steps_" + str(FLAGS.num_steps_gs) + "_num_episodes_" + str(FLAGS.num_episodes_gs)

    # append logs to base path
    log_path = os.path.join(FLAGS.summary_dir_gs, 'grid_search_logs', FLAGS.reward_formulation_gs, exp_name)

    # get the current working directory and append the exp name
    traject_dir = os.path.join(FLAGS.summary_dir_gs, FLAGS.traject_dir_gs, FLAGS.reward_formulation_gs, exp_name)
    ...
    count = 0
    env.reset()
    suggestions = study.suggest(count=flags.FLAGS.num_steps_gs)
