...
flags.DEFINE_float('quantile', 0.5, 'Quantile for the model')
...
flags.DEFINE_enum('solver', 'interior-point', ['highs-ds', 'highs-ipm', 'highs', 'interior-point', 'revised simplex'], 
...
def main(_):
    ...
    if FLAGS.train:
        ...
        # Define the model
        regressor0 = QuantileRegressor(quantile=FLAGS.quantile, alpha=FLAGS.alpha, fit_intercept=FLAGS.fit_intercept, solver=FLAGS.solver)
        
        # Train the model
        regressor0.fit(X_train, y_train[:, 0])

        # Define the model
        regressor1 = QuantileRegressor(quantile=FLAGS.quantile, alpha=FLAGS.alpha, fit_intercept=FLAGS.fit_intercept, solver=FLAGS.solver)
