...
flags.DEFINE_enum('kernel', 'RBF', ['RBF', 'Matern', 'RationalQuadratic', 'ExpSineSquared', 'DotProduct', 'ConstantKernel'], 'Kernel function')
flags.DEFINE_float('alpha', 1.0, 'Regularization parameter')
flags.DEFINE_enum('optimizer', 'fmin_l_bfgs_b', ['fmin_l_bfgs_b'], 'Optimizer')
flags.DEFINE_integer('n_restarts_optimizer', 0, 'Number of restarts for the optimizer')
flags.DEFINE_bool('normalize_y', False, 'Normalize the output')
flags.DEFINE_bool('copy_X_train', True, 'Copy the training data')
...
def main(_):
    # Constraint on kernels
    if FLAGS.kernel == 'RBF':
        kernel = RBF()
    elif FLAGS.kernel == 'Matern':
        kernel = Matern()
    elif FLAGS.kernel == 'RationalQuadratic':
        kernel = RationalQuadratic()
    elif FLAGS.kernel == 'ExpSineSquared':
        kernel = ExpSineSquared()
    elif FLAGS.kernel == 'DotProduct':
        kernel = DotProduct()
    elif FLAGS.kernel == 'ConstantKernel':
        kernel = ConstantKernel()
    else:
        raise ValueError('Kernel function not supported')

    # Define the experiment folder to save the model
    exp_name = 'gaussian_process'
    ...
    if FLAGS.train:
        ...
        # Define the model
        regressor = GaussianProcessRegressor(kernel=kernel, alpha=FLAGS.alpha, optimizer=FLAGS.optimizer, 
                                             n_restarts_optimizer=FLAGS.n_restarts_optimizer, normalize_y=FLAGS.normalize_y, 
                                             copy_X_train=FLAGS.copy_X_train, random_state=FLAGS.random_state)
