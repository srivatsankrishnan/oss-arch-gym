...
flags.DEFINE_float('alpha', 1.0, 'Constant that multiplies the L1 term. Defaults to 1.0')
...
flags.DEFINE_integer('random_state', None, 'The seed of the pseudo random number generator that selects a random feature to update')
...
def main(_):
    ...
    if FLAGS.train:
        ...
        # Define the model
        regressor = MultiTaskLasso(alpha=FLAGS.alpha, fit_intercept=FLAGS.fit_intercept, copy_X=FLAGS.copy_X, 
                                   max_iter=FLAGS.max_iter, tol=FLAGS.tol, warm_start=FLAGS.warm_start, 
                                   random_state=FLAGS.random_state, selection=FLAGS.selection)
