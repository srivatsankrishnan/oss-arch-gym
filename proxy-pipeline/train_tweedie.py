...
flags.DEFINE_float('power', 0, 'Power parameter for the Tweedie distribution')
...
flags.DEFINE_enum('link', 'auto', ['auto', 'identity', 'log'], 'Link function')
flags.DEFINE_integer('max_iter', 100, 'Maximum number of iterations taken for the solvers to converge')
flags.DEFINE_float('tol', 1e-4, 'Tolerance for stopping criteria')
...
def main(_):
    ...
    if FLAGS.train:
        ...
        # Define the model
        regressor0 = TweedieRegressor(power=FLAGS.power, alpha=FLAGS.alpha, fit_intercept=FLAGS.fit_intercept, 
                                      link=FLAGS.link, max_iter=FLAGS.max_iter, tol=FLAGS.tol, 
                                      warm_start=FLAGS.warm_start, verbose=FLAGS.verbose)
        
        # Train the model
        regressor0.fit(X_train, y_train[:, 0])

        # Define the model
        regressor1 = TweedieRegressor(power=FLAGS.power, alpha=FLAGS.alpha, fit_intercept=FLAGS.fit_intercept, 
                                      link=FLAGS.link, max_iter=FLAGS.max_iter, tol=FLAGS.tol, 
                                      warm_start=FLAGS.warm_start, verbose=FLAGS.verbose)
