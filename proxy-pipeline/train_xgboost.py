...
flags.DEFINE_enum('booster', 'gbtree', ['gbtree', 'gblinear', 'dart'], 'Type of booster')
flags.DEFINE_bool('silent', False, 'Whether to print messages while running boosting')
flags.DEFINE_integer('n_thread', 1, 'Number of parallel threads used to run xgboost')
flags.DEFINE_float('eta', 0.3, 'Step size shrinkage used in update to prevents overfitting')
flags.DEFINE_integer('min_child_weight', 1, 'Minimum sum of instance weight (hessian) needed in a child')
flags.DEFINE_integer('max_depth', 6, 'Maximum depth of a tree')
flags.DEFINE_integer('max_leaf_nodes', 0, 'Maximum number of nodes to be added')
flags.DEFINE_float('gamma', 0, 'Minimum loss reduction required to make a further partition on a leaf node of the tree')
flags.DEFINE_float('max_delta_step', 0, 'Maximum delta step we allow each tree’s weight estimation to be')
flags.DEFINE_float('subsample', 1, 'Subsample ratio of the training instance')
flags.DEFINE_float('colsample_bytree', 1, 'Subsample ratio of columns when constructing each tree')
flags.DEFINE_float('lambda_', 1, 'L2 regularization term on weights')
flags.DEFINE_float('alpha', 0, 'L1 regularization term on weights')
flags.DEFINE_float('scale_pos_weight', 1, 'Balancing of positive and negative weights')
flags.DEFINE_enum('eval_metric', 'rmse', ['rmse', 'mae', 'logloss', 'error', 'merror', 'mlogloss', 'auc'], 'Evaluation metric for validation data')
...
def main(_):
    ...
    if FLAGS.train:
        ...
        # Define the model
        regressor = xgb.XGBRegressor(seed=FLAGS.seed, booster=FLAGS.booster, silent=FLAGS.silent, nthread=FLAGS.n_thread,
                                     eta=FLAGS.eta, min_child_weight=FLAGS.min_child_weight, max_depth=FLAGS.max_depth,
                                     max_leaf_nodes=FLAGS.max_leaf_nodes, gamma=FLAGS.gamma, max_delta_step=FLAGS.max_delta_step,
                                     subsample=FLAGS.subsample, colsample_bytree=FLAGS.colsample_bytree, lambda_=FLAGS.lambda_,
                                     alpha=FLAGS.alpha, scale_pos_weight=FLAGS.scale_pos_weight, eval_metric=FLAGS.eval_metric)
