    ...
    if FLAGS.train:
        ...
        for estimator in FLAGS.estimators:
            if os.path.exists(os.path.join(FLAGS.model_path, estimator)):
                if os.path.exists(os.path.join(FLAGS.model_path, estimator, 'model.joblib')):
                    path = os.path.join(FLAGS.model_path, estimator, 'model.joblib')
                    models0.append((estimator, pickle.load(open(path, 'rb'))))
                elif os.path.exists(os.path.join(FLAGS.model_path, estimator, 'model0.joblib')):
                    path = os.path.join(FLAGS.model_path, estimator, 'model0.joblib')
                    models0.append((estimator, pickle.load(open(path, 'rb'))))
                else:
        ...
        regressor0 = VotingRegressor(estimators=models0, n_jobs=FLAGS.n_jobs, verbose=FLAGS.verbose)
        ...
        for estimator in FLAGS.estimators:
            if os.path.exists(os.path.join(FLAGS.model_path, estimator)):
                if os.path.exists(os.path.join(FLAGS.model_path, estimator, 'model.joblib')):
                    path = os.path.join(FLAGS.model_path, estimator, 'model.joblib')
                    models1.append((estimator, pickle.load(open(path, 'rb'))))
                elif os.path.exists(os.path.join(FLAGS.model_path, estimator, 'model1.joblib')):
                    path = os.path.join(FLAGS.model_path, estimator, 'model1.joblib')
                    models1.append((estimator, pickle.load(open(path, 'rb'))))
                else:
                    raise Exception('Model {} is not found'.format(estimator))
            else:
        ...
        regressor1 = VotingRegressor(estimators=models1, n_jobs=FLAGS.n_jobs, verbose=FLAGS.verbose)
