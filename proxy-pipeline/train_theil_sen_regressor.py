...
flags.DEFINE_integer('max_subpopulation', 10000, 'maximum number of subpopulations to consider')
flags.DEFINE_integer('n_subsamples', None, 'maximum number of subpopulations to consider')
flags.DEFINE_integer('max_iter', 300, 'maximum number of iterations for the solver to converge')
flags.DEFINE_float('tol', 1e-3, 'tolerance for the optimization')
...
flags.DEFINE_bool('verbose', False, 'Verbose mode when fiting the model')
...
def main(_):
    ...
    if FLAGS.train:
        ...
        # Define the model
        regressor0 = TheilSenRegressor(fit_intercept=FLAGS.fit_intercept, copy_X=FLAGS.copy_X, 
                                       max_subpopulation=FLAGS.max_subpopulation, n_subsamples=FLAGS.n_subsamples,
                                       max_iter=FLAGS.max_iter, tol=FLAGS.tol, random_state=FLAGS.random_state,
                                       n_jobs=FLAGS.n_jobs, verbose=FLAGS.verbose)
        
        # Train the model
        regressor0.fit(X_train, y_train[:, 0])

        # Define the model
        regressor1 = TheilSenRegressor(fit_intercept=FLAGS.fit_intercept, copy_X=FLAGS.copy_X, 
                                       max_subpopulation=FLAGS.max_subpopulation, n_subsamples=FLAGS.n_subsamples,
                                       max_iter=FLAGS.max_iter, tol=FLAGS.tol, random_state=FLAGS.random_state,
                                       n_jobs=FLAGS.n_jobs, verbose=FLAGS.verbose)
