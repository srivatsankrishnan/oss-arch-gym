...
flags.DEFINE_enum('splitter', 'best', ['best', 'random'], 'The strategy used to choose the split at each node')
flags.DEFINE_integer('max_depth', None, 'The maximum depth of the tree')
...
flags.DEFINE_enum('max_features', None, ['auto', 'sqrt', 'log2'], 'The number of features to consider when looking for the best split')
flags.DEFINE_integer('random_state', None, 'Controls the randomness of the estimator')
flags.DEFINE_integer('max_leaf_nodes', None, 'Grow a tree with max_leaf_nodes in best-first fashion')
...
def main(_):
    ...
    if FLAGS.train:
        ...
        # Define the model
        regressor = DecisionTreeRegressor(criterion=FLAGS.criterion, splitter=FLAGS.splitter, max_depth=FLAGS.max_depth,
