...
flags.DEFINE_bool('custom_dataset', False, 'Whether to use custom dataset or not')

# Hyperparameters for the model
flags.DEFINE_float('alpha', 1, 'Regularization strength')
flags.DEFINE_string('kernel', 'linear', 'Kernel mapping used internally')
flags.DEFINE_float('gamma', None, 'Gamma parameter for the RBF, laplacian, polynomial, exponential chi2 and sigmoid kernels')
flags.DEFINE_integer('degree', 3, 'Degree of the polynomial kernel')
flags.DEFINE_float('coef0', 1, 'Zero coefficient for polynomial and sigmoid kernels')
flags.DEFINE_spaceseplist('kernel_params_keys', None, 'Additional parameters (keyword arguments) for kernel function passed as callable object(keys)')
flags.DEFINE_spaceseplist('kernel_params_values', None, 'Additional parameters (keyword arguments) for kernel function passed as callable object(values)')
...
def main(_):
    # Handle dictionary hyperparameter
    kernel_params = {}
    if FLAGS.kernel_params_keys is not None:
        for i in range(len(FLAGS.kernel_params_keys)):
            kernel_params[FLAGS.kernel_params_keys[i]] = FLAGS.kernel_params_values[i]
    
    # Define the experiment folder to save the model
    exp_name = 'kernel_ridge'
    ...
    if FLAGS.train:
        ...
        # Define the model
        regressor = KernelRidge(alpha=FLAGS.alpha, kernel=FLAGS.kernel, gamma=FLAGS.gamma, degree=FLAGS.degree, coef0=FLAGS.coef0, kernel_params=kernel_params)
