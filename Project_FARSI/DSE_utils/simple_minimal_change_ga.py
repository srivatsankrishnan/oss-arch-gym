...
#                      probability)
bus_DB = range(0, 20)
memory_DB = range(20, 40)
ip_DB = range(40, 600)
block_to_choose_from = ["ip", "memory", "bus"]

def individual_att_generator():
    global bus_DB
    global memory_DB
    global ip_DB
    time.sleep(.00001)
    ...
    memory = random.choice(memory_DB)
    ip = random.choice(ip_DB)
    bus = random.choice(bus_DB)
    return [ip, memory, bus]

toolbox.register("attr_bool", individual_att_generator)
...
def eval_individual_dp(individual):
    # call OSSIM individual is a design point
    return [max(individual[0])]
#----------
# Operator registration
#----------
# register the goal / fitness function
toolbox.register("evaluate", eval_individual_dp)
...
def mate_dp(individual1, individual2):
    ...
    individual2[0][rand_idx] = individual1_el

toolbox.register("mate", mate_dp)
...
toolbox.register("mutate", mutate_dp, indpb=0.05)
...
def main():
    ...
    pop = toolbox.population(n=max_pop_count)
