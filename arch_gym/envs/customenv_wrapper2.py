...
class CustomEnvWrapper(dm_env.Environment):
  ...
  def step(self, action: types.NestedArray) -> dm_env.TimeStep:
    ...
    reward = tree.map_structure(
        ...
        reward,
        self.reward_spec())
    print("aaaaaaddddddddddddddyyyyyyyyyyyyyyyyyyyyyyaaaaaaaaaaaa")
    print("This reward:", reward)

    if done:
      truncated = info.get('TimeLimit.truncated', False)
      if truncated:
        print("type1", type(dm_env.truncation(reward, observation)))
        return dm_env.truncation(reward, observation)
      print("type2", type(dm_env.termination(reward, observation)))
      return dm_env.termination(reward, observation)
    print("type3", type(dm_env.transition(reward, observation)))
...
def make_custom_env(seed: int = 10000,
                     max_steps: int = 100,
  ...
  """
  environment = CustomEnvWrapper(CustomEnv())
