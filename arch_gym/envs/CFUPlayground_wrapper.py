...
class CFUPlaygroundWrapper(dm_env.Environment):
    ...
    def __init__(self, environment: gym.Env, env_wrapper_sel: str= 'macme'):
    ...
    def step(self,action: types.NestedArray) -> dm_env.TimeStep:
        ...
        if(self.env_wrapper_sel=='macme' or self.env_wrapper_sel=='macme_continuous'):
            ...
            for each_agent_action in action:
                ...
                ## in Astra sim the "info" variable is just the number of steps successfully completed so far ## Kartik
                ## so modified CFUPlaygroundEnv to also return no_steps variable
        ...
        lambda x, t: (  # pylint: disable=g-long-lambda
            t.dtype.type(x)
            if np.isscalar(x) else np.asarray(x, dtype=t.dtype)), reward, self.reward_spec())
